...
def main():
    ...
    # --- 染色体级划分（防泄漏；默认与正文一致，可配置）---
    # 正文默认：train=Chr5；val=Chr3+Chr4；test=Chr1+Chr2
    split_cfg = cfg.get("chr_split") or {
        "train": ["Chr5", "5"],
        "val": ["Chr3", "Chr4", "3", "4"],
        "test": ["Chr1", "Chr2", "1", "2"],
    }
    def _norm_chr(c):
        s = str(c).replace("chr", "Chr")
        return s

    meta["chr_n"] = meta["chr"].map(_norm_chr)
    tr_set = {_norm_chr(c) for c in split_cfg["train"]}
    va_set = {_norm_chr(c) for c in split_cfg["val"]}
    te_set = {_norm_chr(c) for c in split_cfg["test"]}
    # 兼容 Araport11 的 "1"/"Chr1" 等写法
    def _match(ch, pool):
        ch = _norm_chr(ch)
        if ch in pool:
            return True
        bare = ch.replace("Chr", "")
        return bare in pool or f"Chr{bare}" in pool

    split = []
    for ch in meta["chr"]:
        if _match(ch, tr_set):
            split.append("train")
        elif _match(ch, va_set):
            split.append("val")
        elif _match(ch, te_set):
            split.append("test")
        else:
            split.append("train")  # 未列染色体并入 train，避免丢样本
    meta["split"] = split

    # 标签阈值仅用 train split，避免 val/test 泄漏（对齐 PGB 协议）
    train_expr = meta.loc[meta["split"] == "train", "log_expr"]
    thr = float(train_expr.quantile(high_q))
    y = (meta["log_expr"] >= thr).astype(int).values
    print(f">>> train 标签阈值 (q={high_q}): {thr:.6f}; "
          f"pos rate train/val/test = "
          f"{y[meta['split'].eq('train')].mean():.3f}/"
          f"{y[meta['split'].eq('val')].mean():.3f}/"
          f"{y[meta['split'].eq('test')].mean():.3f}")

    print("划分:", dict(meta["split"].value_counts()), "| 染色体 train/val/test:",
          sorted(tr_set), sorted(va_set), sorted(te_set))
    meta = meta.drop(columns=["chr_n"], errors="ignore")
    ...
    with open(os.path.join(out_dir, "params.json"), "w") as f:
        json.dump({"promoter": P, "utr5": U5, "utr3": U3, "terminator": T,
                   "gap": GAP_N, "total_len": int(X.shape[2]),
                   "label_threshold_train": thr, "high_expr_quantile": high_q,
                   "chr_split": {k: sorted(list(v)) for k, v in
                                 (("train", tr_set), ("val", va_set), ("test", te_set))}},
                  f, indent=2)
