...
def crop_seq(seq: str, max_seq_len: int, mode: str = "center") -> str:
    """缩短基因中心窗口。默认居中裁剪；left 仅作对照，不作主消融。"""
    ...
    start = max(0, (len(seq) - max_seq_len) // 2)
    return seq[start:start + max_seq_len]
...
class PGBFastaDataset(Dataset):
    ...
    def __init__(self, base_dir, split, task="binary", max_seq_len=None, median=None,
                 species="arabidopsis_thaliana", crop_mode="center"):
        ...
        self.task = task
        logger.info("[pgb-fasta] %s samples=%d median=%.3f crop=%s",
                    split, len(self.seqs), self.median, crop_mode)
    ...
    def __getitem__(self, i):
        seq = self.seqs[i]
        if self.max_seq_len:
            seq = crop_seq(seq, self.max_seq_len, self.crop_mode)
...
def make_pgb_datasets(base_dir, task="binary", max_seq_len=None, species="arabidopsis_thaliana",
    ...
    tr = PGBFastaDataset(base_dir, "train", task, max_seq_len, species=species, crop_mode=crop_mode)
    median = tr.median
    va = PGBFastaDataset(base_dir, "validation", task, max_seq_len, median=median,
                         species=species, crop_mode=crop_mode)
    te = PGBFastaDataset(base_dir, "test", task, max_seq_len, median=median,
                         species=species, crop_mode=crop_mode)
    return tr, va, te
...
def load_text_splits(cfg, mode="pgb", task="binary", hf_ds=None, max_seq_len=None):
    ...
    if mode == "pgb":
        ...
        tr, va, te = make_pgb_datasets(base, task, max_seq_len, crop_mode=crop_mode)
        out = []
        for ds in (tr, va, te):
...
def make_loaders(cfg, task="binary", mode="pgb", hf_ds=None, batch_size=64,
    ...
    if mode == "pgb":
        ...
        crop_mode = cfg["data"].get("crop_mode", "center")
        tr, va, te = make_pgb_datasets(base, task, max_seq_len=msl, crop_mode=crop_mode)
    else:
        d = cfg["data"]
