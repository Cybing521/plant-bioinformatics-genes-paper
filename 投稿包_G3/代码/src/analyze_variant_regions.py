...
def main():
    ...
    ap.add_argument("--tag", default="", help="输出文件名前缀，如 agront_")
    ap.add_argument("--abs-col", default="abs_delta",
                    help="|Δ| 列名；AgroNT 位点表用 mean_abs_delta")
    args = ap.parse_args()

    df = pd.read_csv(args.scores, sep="\t")
    if args.abs_col not in df.columns and "abs_delta" in df.columns:
        args.abs_col = "abs_delta"
    if args.abs_col != "abs_delta":
        df["abs_delta"] = df[args.abs_col]
    ...
    if os.path.exists(args.eqtl):
        ...
        # Prefer model-specific |Δ| when both CNN abs_delta and agront_abs_delta are present.
        if args.abs_col == "mean_abs_delta" and "agront_abs_delta" in eq.columns:
            eq["abs_delta"] = eq["agront_abs_delta"]
        elif "abs_delta" not in eq.columns:
            for cand in ("agront_abs_delta", "mean_abs_delta"):
                if cand in eq.columns:
                    eq["abs_delta"] = eq[cand]
                    break
        ...
        if len(top) > 10 and "abs_delta" in top.columns:
    ...
    tag = args.tag
    stats_path = os.path.join(args.out_dir, f"{tag}variant_region_stats.csv")
    pair_path = os.path.join(args.out_dir, f"{tag}variant_region_pairwise.csv")
    stats.to_csv(stats_path, index=False)
    pair_df.to_csv(pair_path, index=False)

    summary_path = os.path.join(args.out_dir, f"{tag}variant_region_summary.txt")
    lines = [
    ...
    fig.tight_layout()
    fig_stem = "fig5b_variant_by_region" if not tag else f"fig5b_{tag.rstrip('_')}_variant_by_region"
    fig_path = os.path.join(args.fig_dir, f"{fig_stem}.png")
