...
class AgroNTClassifier(nn.Module):
    def __init__(self, model_id, num_classes=1, task="binary",
        ...
        # 梯度检查点：训练时降显存；推理时关闭以免拖慢前向。
        self._want_grad_ckpt = True
        try:
...
def build_agront(cfg: dict, num_classes=1, task="binary", max_seq_len=None) -> AgroNTClassifier:
    m = cfg["model"]["agront"]
    return AgroNTClassifier(
        ...
        max_seq_len=max_seq_len or cfg.get("agront_train", {}).get("max_seq_len") or m.get("max_bp"),
    )


def load_finetuned_agront(cfg, ckpt_path, device, task="binary", max_seq_len=None):
    """Load LoRA-finetuned AgroNT from a full state_dict checkpoint."""
    model = build_agront(cfg, num_classes=1, task=task, max_seq_len=max_seq_len)
...
def predict_seqs(model, seqs, batch_size=8):
    """Batched sigmoid probabilities for a list of DNA strings."""
    import numpy as np
    out = []
    with torch.no_grad():
        for i in range(0, len(seqs), batch_size):
            chunk = list(seqs[i:i + batch_size])
            logit = model(chunk).squeeze(-1)
            if logit.ndim == 0:
