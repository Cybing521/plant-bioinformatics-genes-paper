...
def map_flank_pos(variants, gff_path, flanking):
    ...
    for _, row in variants.iterrows():
        ...
        # CDS 边界（用于 UTR 定位）
        if len(cds):
            cds_start = int(min(cds["start"]) - 1) if strand == "+" else int(max(cds["end"]))
            cds_end = int(max(cds["end"])) if strand == "+" else int(min(cds["start"]) - 1)
        else:
            cds_start, cds_end = tss, tts
        GAP = 20
        if strand == "+":
            if tss - P <= pos < tss:                       # 启动子
                row["flank_pos"] = pos - (tss - P)
            elif tss <= pos < cds_start:                   # 5'UTR
                row["flank_pos"] = P + (pos - tss)
            elif cds_end < pos <= tts:                     # 3'UTR
                row["flank_pos"] = P + U5 + GAP + (pos - cds_end - 1)
            elif tts < pos <= tts + T:                     # 终止子
                row["flank_pos"] = P + U5 + GAP + U3 + (pos - tts - 1)
            else:
                row["flank_pos"] = np.nan
        else:
            if tss < pos <= tss + P:                       # 启动子
                row["flank_pos"] = P - (pos - tss)
            elif cds_end < pos <= tss:                     # 5'UTR
                row["flank_pos"] = P + (tss - pos)
            elif cds_start <= pos < cds_end:               # 3'UTR
                row["flank_pos"] = P + U5 + GAP + (cds_end - pos - 1)
            elif tts - T <= pos < tts:                     # 终止子
                row["flank_pos"] = P + U5 + GAP + U3 + (tts - pos - 1)
            else:
                row["flank_pos"] = np.nan
    return variants
